...
const parseInline = (str) => {
  ...
  while ((match = regex.exec(str)) !== null) {
    ...
    if (match[1])
      ...
      parts.push(
        <em key={match.index} className="italic text-white/55">
          {match[2]}
        </em>,
      );
    ...
    <ul className="space-y-2 list-none">
      {lines.map((line, i) => {
        ...
        return isBullet ? (
          ...
          <p key={i} className="text-white/45 text-xs mt-1 italic">
            {parseInline(content)}
          </p>
        );
      })}
...
const MovieChatBot = () => {
  ...
  const botIconKey = useSelector(
    (store) => store.user?.botIcon || DEFAULT_BOT_ICON,
  );
  const BotIcon = BOT_ICON_MAP[botIconKey] || BOT_ICON_MAP[DEFAULT_BOT_ICON];
  const { fetchSessions, createSession, saveSession, deleteSession } =
    useChatHistory();
  ...
  const handleSend = async (text) => {
    ...
    try {
      ...
      const title =
        existingSession?.title || userText.slice(0, SESSION_TITLE_MAX_LEN);
      saveSession(
        sessionId,
              ...
              {sessions.length === 0 ? (
                <div className="flex-1 flex flex-col items-center justify-center gap-3 py-10">
                  <div className="w-12 h-12 rounded-2xl bg-white/6 flex items-center justify-center">
                    <BotIcon size={24} color="rgba(255,255,255,0.4)" />
                  </div>
                ...
                <div className="space-y-1.5">
                  <p className="text-white/25 text-[10px] uppercase tracking-widest px-1 mb-2">
                    Recent
                  </p>
                  {sessions.map((session) => (
                    ...
                    >
                      <div className="flex gap-2.5 items-start flex-1 min-w-0">
                        <div className="w-7 h-7 rounded-lg bg-white/8 flex items-center justify-center shrink-0 mt-0.5 text-sm">
                          🎞️
                        </div>
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-white/22 text-[9px] mt-1">
                      ...
                      <button
                        ...
                        className="opacity-100 sm:opacity-0 sm:group-hover:opacity-100 text-white/25 hover:text-red-400 transition-all p-1 shrink-0 mt-0.5 rounded"
                      >
                        <svg
                    ...
                    <div
                      ...
                      style={
                        msg.role === "user"
                          ...
                          : {
                              background: CHAT_THEME.botMsgBg,
                              borderColor: CHAT_THEME.borderColor,
                            }
                      }
                    >
